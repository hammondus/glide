...
| `Error` | the dynamic error type application code uses: `fn run() -> Result<(), Error>`. **Anything is assignable to it** — that is the whole point, and it is why `Err("config is empty")` needs no ceremony and why `?` propagates any callee's error into it with no `from`. `.context(msg)` on a `Result` adds a breadcrumb and prints as a chain. **Erased**, so an `Error` slot holds whatever it was given: the type carries **no methods** ○, and `println(r)` on a program-made error shows `Err("msg")` where a host error shows `Err(msg)` — unwrap and interpolate (`Err(e) => "{e}"`) and the two read alike. See DESIGN.md's open question on boxing it | ✓ |
