...
| `Error` | the dynamic error type application code uses: `fn run() -> Result<(), Error>`. **Anything is assignable to it** — that is the whole point, and it is why `Err("config is empty")` needs no ceremony and why `?` propagates any callee's error into it with no `from`. Erased at the *type* level, **boxed** at the *value* level: what lands in an `Error` slot becomes an `Error`, so a program-made error and a host one are the same kind of thing and print alike. Methods: `message()` (this link only), `cause() -> Error?`, `context(msg)`, and `find(SomeType) -> SomeType?` which walks the cause chain for a concrete typed error. Boxing is idempotent, so coercing an `Error` into an `Error` never double-wraps. A **variant pattern cannot match an `Error`** — the concrete error is inside the box — and that is a reported error naming `find`, never a silently dead arm | ✓ |
