# Chapter 25: Structured Concurrency
...
make the model. Cancellation gets Chapter 26; channels get Chapter 27.
...
One compile-time restriction, designed and not yet enforced:
...
in completion order is `select` territory (Chapter 28).
...
the two-primitive split (above), `Job.cancel()` (Chapter 26), and
...
server's error, and a `defer cancel()`. Chapter 26 does that comparison
properly.
...
- Interpreter: one lock, released around blocking operations, so tasks
  ...
  this tier.
- ○: the rule that task-crossing closures may not capture `mut`
  bindings.
