# Chapter 40: Architecture and Project Structure
...
Files within a module share one namespace (Chapter 30), so splitting is
editorial:
...
**No globals** (Chapter 30) — so dependencies must be passed, so they
...
**No reflection** (Chapter 36) — so a container *cannot* autowire, and
the honest alternative is the only alternative.

**Scopes own lifetimes** (Chapter 26) — so "when is this shut down" has
...
— the dogfood rule (Chapter 31) applied to the standard library, and it
...
Glide has no ORM, permanently (Chapter 35). `db.query` returns rows;
