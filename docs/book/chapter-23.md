...
`Iterator`. Chapter 24 covers how you *write* one.
...
`Range`; a generator function (Chapter 24); or any type with an
...
uses `yield` — that is a generator, and Chapter 24 explains it.
...
- **Generators are sugar for the same protocol**, not a second one, so
  they compose with everything (Chapter 24).
